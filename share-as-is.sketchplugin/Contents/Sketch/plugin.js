...
var _rasterize = __webpack_require__(8);

var _rasterize2 = _interopRequireDefault(_rasterize);
...
var SystemSounds = exports.SystemSounds = {
  Basso: "Basso",
  Blow: "Blow",
  Bottle: "Bottle",
  Frog: "Frog",
...
var _sketchColor = __webpack_require__(9);
...
var colorToNSColor = function colorToNSColor(color) {
  var rgba = (0, _tinycolor2['default'])(color).toRgb();
  return NSColor.colorWithRed_green_blue_alpha(rgba.r / 255.0, rgba.g / 255.0, rgba.b / 255.0, rgba.a);
};

var addArtboardBlock = function addArtboardBlock(page, artboard) {
  var useCustomLabelsForSymbols = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;

  var shape = MSShapeGroup.shapeWithRect(artboard.rect());
  shape.style().addStylePartOfType(0).color = (0, _sketchColor2['default'])('white');

  var shadow = shape.style().addStylePartOfType(2);
  shadow.offsetX = 0;
  shadow.offsetY = 1;
  shadow.color = (0, _sketchColor2['default'])('rgba(0,0,0,0.3)');
  shadow.blurRadius = 3;
  ...
  label.textBehaviour = _sketchConstants.TextBehaviour.Fixed;
  ...
  var labelColor = artboard.isKindOfClass(MSSymbolMaster) && useCustomLabelsForSymbols ? '#7B20A5' : '#5F5F5F';
  var attributes = {
...
var rasterize = function rasterize(document, selection) {
  var options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};

  var documentData = document.documentData().copyWithOptions(1);
  var page = documentData.currentPage();
  if (page.layers().count() < 1) {
    return null;
  }

  var artboards = NSArray.arrayWithArray([]);
  var rect = null;

  var exportSelectedLayersOnly = false;
  ...
  if (selection.count() < 1) {
    ...
    var originalArtboards = selection.valueForKeyPath('@distinctUnionOfObjects.parentArtboard');
    var shadowArtboards = [];
    _lodash2['default'].each(_utils2['default'].normalize(originalArtboards), function (artboard) {
      var shadowArtboard = documentData.layerWithID(artboard.objectID());
      if (shadowArtboard) {
    ...
    _lodash2['default'].each(_utils2['default'].normalize(artboards), function (artboard) {
      artboard.includeBackgroundColorInExport = true;
    });

    var _layers = artboards;
    for (var _i = 0; _i < _layers.count(); _i++) {
      var _layer = _layers.objectAtIndex(_i);
      rect = !rect ? _layer.absoluteInfluenceRect() : CGRectUnion(_layer.absoluteInfluenceRect(), rect);
    }
  ...
  shape.style().addStylePartOfType(0).color = (0, _sketchColor2['default'])('#F2F2F2');

  page.insertLayer_atIndex(shape, 0);

  // Add artboard mocks
  var boards = _lodash2['default'].map(artboards, function (board) {
    return board;
  });
  var addedLayers = [];
  _lodash2['default'].each(boards, function (board) {
    if (board.isKindOfClass(MSArtboardGroup)) {
      addedLayers = addedLayers.concat(addArtboardBlock(page, board));
    }
  });

  var request = MSExportRequest.alloc().init();
  request.rect = rect;
  request.scale = 1;
  ...
  if (exportSelectedLayersOnly) {
    var includedLayerIDs = NSMutableSet.alloc().init();
    _lodash2['default'].each(boards.concat(addedLayers).concat([shape]), function (board) {
      var objectIDs = request.objectIDsForSelfAncestorsAndChildrenOfAncestry(MSImmutableLayerAncestry.ancestryWithMSLayer(board));
  ...
  var exporter = MSExporter.exporterForRequest_colorSpace(request, NSColorSpace.sRGBColorSpace());
  return exporter.image();
};

exports['default'] = rasterize;
...
var isMSColorInstance = function isMSColorInstance(obj) {
    return toString.call(obj) === '[object MOBoxedObject]' && obj.className() == 'MSColor';
};

var convert = function convert(input, fn, clr) {
    clr = clr || MSColor.alloc().init();
    var rgba = fn(input).toRgb();
...
function color(input, clr) {
    if (isMSColorInstance(input)) {
        return _tinycolor2.default.fromRatio({
            r: input.red(),
            g: input.green(),
            b: input.blue(),
            a: input.alpha()
    ...
    return convert(input, _tinycolor2.default, clr);
}

function colorFromRatio(input, clr) {
    return convert(input, _tinycolor2.default.fromRatio, clr);
}
...
var logEvent = exports.logEvent = function logEvent(event, props) {
  ...
  if (!uuid) {
    ...
    NSUserDefaults.standardUserDefaults().setObject_forKey(uuid, _constants.UUIDKey);
  }
